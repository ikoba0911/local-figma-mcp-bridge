...
- ローカル bridge は現在 WebSocket client を認証していません。信頼できるマシン上でのみ実行し、独自に認証や transport protection を追加していない限り host は `localhost` に固定してください。
...
Figma Plugin は JavaScript 環境で動くため、引き続き TypeScript でビルドします。ローカル MCP サーバは Go 1.25 で実装されており、Go module の外部依存がない単一バイナリとしてビルドできます。

## ローカルブリッジの起動

WebSocket bridge をローカル常駐プロセスとして起動します。

```bash
./bin/local-figma-mcp-bridge bridge
```

この bridge は Figma Plugin 用に `ws://localhost:8787` を listen します。MCP client からは同じバイナリを引数なしで起動します。その場合、ローカルポートは listen せず、起動済み bridge へ接続するだけです。
...
5. ローカル bridge が起動していると、Plugin UI が自動的に接続します

UI が `Disconnected` と表示される場合は、ローカル bridge を起動して Plugin の自動再接続を待ってください。
...
MCP stdio process は `ws://localhost:8787/mcp` で bridge に接続します。MCP stdio process 自体はローカルポートを listen しません。
